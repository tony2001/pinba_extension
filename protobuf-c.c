...
#else
# define UNPACK_ERROR(args)  do { if (protobuf_c_error_printer) protobuf_c_error_printer args; } while(0)
#endif
                                   ...
                                   const uint8_t       *data)
{
  ...
  const unsigned word_bits = sizeof(long) * 8;
  ...
  while (rem > 0)
    {
      ...
      if (used == 0)
        {
          UNPACK_ERROR (("message '%s': error parsing tag/wiretype at offset %u", desc->name,
                         (unsigned)(at-data)));
      ...
      switch (wire_type)
        ...
        case PROTOBUF_C_WIRE_TYPE_VARINT:
          {
            ...
            if (i == max_len)
              {
                UNPACK_ERROR (("message '%s', field '%s': unterminated varint at offset %u",
                               desc->name, field ? field->name : "*unknown*",
                               (unsigned)(at-data)));
                goto error_cleanup_during_scan;
              }
            tmp.len = i + 1;
        ...
        case PROTOBUF_C_WIRE_TYPE_64BIT:
          if (rem < 8)
            {
              UNPACK_ERROR (("message '%s', field '%s': too short after 64bit wiretype at offset %u",
                             desc->name, field ? field->name : "*unknown*",
                             (unsigned)(at-data)));
              goto error_cleanup_during_scan;
            }
        ...
        case PROTOBUF_C_WIRE_TYPE_LENGTH_PREFIXED:
          {
            ...
            if (tmp.len == 0)
              {
                UNPACK_ERROR (("message '%s', field '%s': scan of length prefixed data failed",
                               desc->name, field ? field->name : "*unknown*"));
                goto error_cleanup_during_scan;
              }
        ...
        case PROTOBUF_C_WIRE_TYPE_32BIT:
          if (rem < 4)
            {
              UNPACK_ERROR (("message '%s', field '%s': too short after 32bit wiretype at offset %u",
                             desc->name, field ? field->name : "*unknown*",
                             (unsigned)(at-data)));
              goto error_cleanup_during_scan;
            }
          ...
          break;
        default:
          UNPACK_ERROR (("message '%s', field '%s': unsupported tag %u at offset %u",
                         desc->name, field ? field->name : "*unknown*",
      ...
      if (in_slab_index == (1U<<(which_slab+FIRST_SCANNED_MEMBER_SLAB_SIZE_LOG2)))
        {
          ...
          if (which_slab == MAX_SCANNED_MEMBER_SLAB)
            {
              UNPACK_ERROR (("message '%s', field '%s': too many fields", desc->name, field ? field->name : "*unknown*"));
              goto error_cleanup_during_scan;
            }
          which_slab++;
      ...
      for (j = 0; j < max; j++)
        {
          if (!parse_member (slab + j, rv, allocator))
            {
              UNPACK_ERROR (("message '%s', field '%s': error parsing member",
                             desc->name, slab->field ? slab->field->name : "*unknown*"));
              goto error_cleanup;
            }
        }
    }
